...
dat <-
  read.csv('D:/BG_SHP/state_level_variables_to_R1.csv')
...
all_dat <- data.frame()
all_dat_1 <- data.frame()
for (ii in 1:1000){
  print (ii)
  bootstrap_sample <-sample_n(dat, nrow(dat),replace = T)
  GAM_RES1 <-
    bam(
      ...
      select = TRUE,
      family = c("gaussian"),
      method = "fREML",discrete = TRUE, chunk.size=5000, nthreads = 200
    )
  summary_table <- summary(GAM_RES1)
  summary_table<-data.frame(summary_table$p.table)
  summary_table$ID <- ii
  all_dat<-rbind(all_dat,summary_table)
  ...
  summary_table <- summary(GAM_RES2)
  summary_table<-data.frame(summary_table$p.table)
  summary_table$ID <- ii
  all_dat_1<-rbind(all_dat_1,summary_table)
}
# GAM

write.csv(all_dat, 'D:/BG_SHP/all_dat_ttest__R1.csv')
write.csv(all_dat_1, 'D:/BG_SHP/all_dat1_ttest__R1.csv')

# T-TEST
t.test(all_dat[c(FALSE,TRUE,rep(FALSE,7)), 1],all_dat[c(FALSE,FALSE,TRUE,rep(FALSE,6)), 1],paired=TRUE)
t.test(all_dat[c(FALSE,FALSE,TRUE,rep(FALSE,6)), 1],all_dat[c(FALSE,FALSE,FALSE,TRUE,rep(FALSE,5)), 1],paired=TRUE)
#t.test(all_dat[c(FALSE,TRUE,rep(FALSE,7)), 1],all_dat[c(FALSE,FALSE,FALSE,TRUE,rep(FALSE,5)), 1],paired=TRUE)

t.test(all_dat_1[c(FALSE,TRUE,rep(FALSE,7)), 1],all_dat_1[c(FALSE,FALSE,TRUE,rep(FALSE,6)), 1],paired=TRUE)
t.test(all_dat_1[c(FALSE,FALSE,TRUE,rep(FALSE,6)), 1],all_dat_1[c(FALSE,FALSE,FALSE,TRUE,rep(FALSE,5)), 1],paired=TRUE)
